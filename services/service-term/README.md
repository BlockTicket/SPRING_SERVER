...
처음 실행한 DB에서는 `data: []`가 반환됩니다. IntelliJ HTTP Client의 `requests.http`로
공개 조회와 인증 없는 변경 요청이 401로 차단되는 것을 확인할 수 있습니다.

## 약관 변경 권한

| 요청 | 필요한 권한 |
| --- | --- |
| `GET /api/terms`, `GET /api/term/{id}` | 누구나 조회 가능 |
| `POST /api/term`, `PATCH /api/term`, `DELETE /api/term/{id}` | 인증된 `ADMIN` |

`TermSecurityConfig`가 약관 서버에서 직접 검사하므로 Gateway를 거치지 않은 요청에도 적용됩니다.
인증되지 않은 변경 요청은 401 / `TERM_AUTHENTICATION_REQUIRED`,
인증되었지만 관리자 권한이 없는 요청은 403 / `TERM_ACCESS_DENIED`를 반환합니다.
이 오류는 기존 `CommonResponse`의 `httpStatus`, `message`, `code` 형식을 사용합니다.

**현재 로그인·JWT 인증 연동은 아직 없습니다.** 따라서 일반 실행 환경의 변경 요청은 401로 차단됩니다.
팀의 인증 코드가 검증한 사용자에게 `ROLE_ADMIN`을 부여하도록 연결해야 실제 관리자가 변경할 수 있습니다.
`hasRole("ADMIN")`은 이 서버 측 권한을 검사하며, 요청 body나 `X-Role` 헤더로 권한을 지정할 수 없습니다.
폼 로그인·HTTP Basic·세션 인증은 활성화하지 않았고 `dev` 프로필도 같은 권한 규칙을 적용합니다.
나중에 쿠키 기반 로그인을 연결한다면 현재 비활성화한 CSRF 보호 설정도 함께 검토해야 합니다.
...
Gateway 설정은 **로컬 연결 확인용**입니다. 약관 서버의 관리자 권한 검사는 적용되지만,
실제 로그인·JWT 인증 연동과 운영 Gateway 경로는 팀 정책을 확정한 뒤 별도로 반영해야 합니다.

## 테스트

기존 단위 테스트, API 계약 테스트, 보안 필터를 적용한 권한 테스트는 Docker 없이 실행합니다.

```sh
./gradlew :services:service-term:test
```

실제 MySQL을 사용하는 통합 테스트는 별도 명령으로 실행합니다.
...
인증 연동 전이므로 관리자 변경 요청은 MockMvc의 테스트 사용자(`ADMIN`)로 실행합니다.
이때도 실제 보안 필터·컨트롤러·서비스·MySQL을 거치며, 보안 필터를 끄지 않습니다.
공개 조회와 인증 없는 변경 요청 차단은 실제 HTTP 서버로 검증합니다.
이는 로그인이나 JWT 검증 자체를 테스트한 결과는 아닙니다.
...
- 테스트 관리자로 등록·수정·삭제하고 실제 HTTP로 조회 및 삭제 후 404 확인
- 255자 한국어 제목과 긴 한국어 본문이 MySQL에 저장되는지 SQL로 확인
- 256자 제목이 400으로 거절되고 DB에 저장되지 않는지 확인
- 실제 HTTP의 미인증 등록·수정·삭제가 모두 401이며 DB 내용이 유지되는지 확인
...
- 입력 검증 오류의 공통 응답 형식
- 운영 Gateway 경로 (`dev` 로컬 연결만 구현)
- 로그인·JWT 인증 결과와 관리자 권한 연결 (`ROLE_ADMIN` 권한 검사 구현)
